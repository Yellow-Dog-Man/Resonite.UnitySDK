...
{
    Elements = ExcludedListeners.Select(m => new ResoniteLink.Reference() { }).ToList<ResoniteLink.Member>()
});
members.Add("excludedUsers", new ResoniteLink.SyncList()
{
    Elements = excludedUsers.Select(m => new ResoniteLink.Reference() { }).ToList<ResoniteLink.Member>()
});
}
