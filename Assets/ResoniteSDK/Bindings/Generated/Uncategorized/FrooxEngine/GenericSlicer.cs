...
{
    Elements = Targets.Select(m => new ResoniteLink.SyncObject() { Members = m.CollectMembers() }).ToList<ResoniteLink.Member>()
});
members.Add("Space", new ResoniteLink.SyncObject() { Members = Space.CollectMembers() });
members.Add("_grabbedVisual", new ResoniteLink.Reference() { });
}
