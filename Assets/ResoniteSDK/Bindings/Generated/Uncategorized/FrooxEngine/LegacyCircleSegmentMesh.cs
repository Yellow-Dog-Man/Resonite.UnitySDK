...
{
    Elements = CircleItemOverrides.Select(m => new ResoniteLink.SyncObject() { Members = m.CollectMembers() }).ToList<ResoniteLink.Member>()
});
members.Add("IndependentSegments", new ResoniteLink.SyncList()
{
    Elements = IndependentSegments.Select(m => new ResoniteLink.SyncObject() { Members = m.CollectMembers() }).ToList<ResoniteLink.Member>()
});
