...
{
    Elements = BlockedTypes.Select(m => m.ToResoniteLinkField()).ToList<ResoniteLink.Member>()
});
members.Add("RequiredTypes", new ResoniteLink.SyncList()
{
    Elements = RequiredTypes.Select(m => m.ToResoniteLinkField()).ToList<ResoniteLink.Member>()
});
}
