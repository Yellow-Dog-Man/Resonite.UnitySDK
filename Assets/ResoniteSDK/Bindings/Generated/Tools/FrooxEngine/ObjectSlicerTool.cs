...
{
    Elements = _slicers.Select(m => new ResoniteLink.Reference() { }).ToList<ResoniteLink.Member>()
});
members.Add("_sliceMaterials", new ResoniteLink.SyncList()
{
    Elements = _sliceMaterials.Select(m => new ResoniteLink.Reference() { }).ToList<ResoniteLink.Member>()
});
}
